...
TRANS_PATH = "../input/toxictranslationdata/train_de_en_dropdup.csv"
identity_columns = [
...
exp = "exp8_bert_trans"
model_path = "../input/toxicfittedmodels/exp8_bert_trans_epoch0_fold{}.pth".format(fold_id)
seed_torch(seed)
...
def main():
    ...
    train_df["fold_id"] = fold_df["fold_id"].values

    trasn_df = pd.read_csv(TRANS_PATH)
    trasn_df = pd.merge(train_df[["id", "fold_id"]], trasn_df, how="left", on="id")
    train_df.drop("fold_id", axis=1, inplace=True)

    trans_train_index = trasn_df.fold_id != fold_id
    trasn_df = trasn_df[trans_train_index].sample(frac=0.5, random_state=2019)
    train_df = train_df.append(trasn_df).reset_index(drop=True)
    del trans_train_index, trasn_df
    gc.collect()
    ...
    X_trans = X_text[n_train_df:].astype("int32")
    X_text = X_text[:n_train_df].astype("int32")
    w_trans = weights[n_train_df:].astype("float32")
    weights = weights[:n_train_df].astype("float32")
    y_trans = y[n_train_df:].astype("float32")
    y = y[:n_train_df].astype("float32")
    y_aux_trans = y_aux[n_train_df:].astype("float32")
    y_aux = y_aux[:n_train_df].astype("float32")
    ...
    with timer('train'):
        ...
        y_trans = np.concatenate((y_trans.reshape(-1, 1), w_trans.reshape(-1, 1), y_aux_trans), axis=1).astype(
            "float32")
