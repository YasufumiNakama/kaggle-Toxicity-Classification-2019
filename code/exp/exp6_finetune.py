...
from utils_gpt import seed_torch, convert_lines_gpt2, setting, trim_tensors
...
#bert_config = GPT2Config('../input/gpt2-output/bert_config.json')
...
exp = "exp6"
model_path = "../input/gpt2-output/exp6_gpt2_epoch0_fold{}.pth".format(fold_id)
seed_torch(seed)
...
def main():
    ...
    with timer('load embedding'):
        ...
        X_text = convert_lines_gpt2(train_df['comment_text'].fillna("DUMMY_VALUE").values, max_len, tokenizer)
    ...
    with timer('train'):
        ...
        model = GPT2ClassificationHeadModel(bert_config, n_class=n_labels)
        ...
        for epoch in range(1, epochs+1):
            ...
            torch.save(model.state_dict(), '{}_gpt2_epoch{}_fold{}.pth'.format(exp, epoch, fold_id))
