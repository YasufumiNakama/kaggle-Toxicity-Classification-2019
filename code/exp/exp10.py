...
FOLD_PATH_QUORA = "../input/quora-folds/quora_folds.csv"
QUORA_PATH = "../input/quora-insincere-questions-classification/train.csv"
...
def main():
    ...
    quora_folds = pd.read_csv(FOLD_PATH_QUORA)

    quora_df = pd.read_csv(QUORA_PATH)
    quora_df = quora_df[quora_folds.fold_id != fold_id]
    train_df = train_df.append(quora_df).reset_index(drop=True)
    del quora_folds, quora_df
    gc.collect()
